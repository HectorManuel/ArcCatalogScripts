...
#
# NOTE: El archivo de scratch.gbd debe ubicarse y agregar el path en el file del toolbox antes de
# der publicado, Este file estaba colocado de este modo desde un principio. simplemente modifique el metodo para
# que fuera mas facil ubicarlo y una ves en el servidor lo lea y lo suba no vuelve a utilizarlo por el resto del codigo, ya
# que lo sube a un scratch folder desde donde lo digue llamando. funciona igual que el Zip fiel que contiene los CAD.
# ---------------------------------------------------------------------------
...
# Description: 
# calcula los campos de las tablas y los procesa.
# ---------------------------------------------------------------------------
...
try:
    ...
    try:
        ...
        e = sys.exc_info()[1]
        print(e.args[0])
        print("Error creando paths para el servicio")
        arcpy.AddMessage(e.args[0])
    ...
    #Process structure: arcpy.NameOfTheProcess_AliasOfTheToolbox , this is used if there is a toolbox in use

    #process: Create the Db connection
    try:
        #Creates the DAtaBase connection and return the output path
        ...
        e = sys.exc_info()[1]
        print(e.args[0])
        print("Error creando conección base datos")
        arcpy.AddMessage(e.args[0])

    #process: uncompress Zip
    #call the function to uncompress the zip file
    try:
